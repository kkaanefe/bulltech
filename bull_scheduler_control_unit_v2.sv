module bull_scheduler_control_unit #(
...
parameter int SHIFT = 9
    ...
    taxi_axis_if.snk s_axis, // TS_FIFO dan gelen axi stream
    taxi_axis_if.mon mon_axis, // gate ve frame_FIFO arasındaki axi stream
...
always_ff@(posedge clk) begin 
    ...
    if(rst) begin 
        ...
        WAIT:begin 
            ...
            if(cycle_cnt >= due_time_reg) begin 
...
always_ff@(posedge clk) begin 
    ...
    else if((state == SEND) && s_axis_handshake) begin // burada treadyi açıp handshake gerçekleştikten sonra kapatılmalı
        
        ts_next <= s_axis.tdata;
        ts_next_valid <= 1'b1;
    end
    
    else if((state == SEND) && ts_next_valid) begin
    ...
    else if((state == STOP) && s_axis_handshake) begin// tvalid 1 e çekilirse treadyini 1 e çekiceksin  
        
        ts_next <= s_axis.tdata;
        ts_next_valid <= 1'b1;
    end

    else if((state == STOP ) && ts_next_valid) begin
...
always_comb begin 
    ...
    else if((state == IDLE) && s_axis.tvalid)
        s_axis.tready = 1'b1;
        
    else if((state == SEND) && s_axis.tvalid && !ts_next_valid && !due_next_valid)
        s_axis.tready = 1'b1;

    else if((state == STOP) && s_axis.tvalid && !ts_next_valid && !due_next_valid)
        s_axis.tready = 1'b1;
